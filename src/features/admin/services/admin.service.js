...
import { comparePassword, hashPassword } from "../../../utils/password.config.js";
...
const adminService = {
    create: async (data) => {
        try {
            ...
            if (!userName || !password) {
                throw new AppError("Username and password are required", httpCode.BAD_REQUEST);
            }
            ...
            if (!admin) {
                throw new AppError("Failed to create admin", httpCode.INTERNAL_SERVER_ERROR);
            }
            const token = generateToken(admin._id, admin.email, 'admin');
            return token;
        } catch (error) {
            throw new AppError(error.message, error.statusCode || httpCode.INTERNAL_SERVER_ERROR);
        }
    },
    ...
    update: async (id, data) => {
        try {
            ...
            const admin = await AdminModel.findByIdAndUpdate(id, { 'password': hashedPassword }, { new: true }).select('-password');
            if (!admin) {
                throw new AppError("Admin not found", httpCode.NOT_FOUND);
            }
            return admin;
